...
App = {
  ...
  loadWeb3: async () => {
    ...
    if (window.ethereum) {
      ...
      try {
        ...
        await await ethereum.requestAccounts();
      } catch (error) {
  ...
  createProposal: async () => {
    try {
        ...
        await App.contract.methods.createProposal(amount, time, mortgage).send({
            from: App.account,
            gas: 7000000 , gasPrice:20000000000// 1 Gwei
        });
  ...
  getLoan: async () => {
    ...
    loans.forEach(async (loan) => {
      ...
      console.log(loanAmount);
          ...
          const receipt = await App.contract.methods.acceptLender(loan.loanId, loan.proposalId).send({from: App.account , gas: 7000000 , gasPrice:20000000000});
          console.log(receipt);
  ...
  //   },
    ...
    repay : async (id) => {
      ...
      all_potential_lenders.forEach(async lender => {
        if(lender.loanId == id){ 
        await App.contract.methods.loanPaid(lender.loanId).send({from : App.account , gas: 3000000 , gasPrice:1000000000});
         ...
         let val = "";
         val+=final;
         await web3.eth.sendTransaction({ from: App.account, to: lender.lender, value: web3.utils.toWei(val, 'ether') , gas: 7000000 , gasPrice:20000000000}).then(async function(receipt){
           console.log(receipt);
          App.bool = false;
        });
        }
